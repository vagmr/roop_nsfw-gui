...
def toggle_processor(value: bool):
    if value:
...
def create_root(start: Callable[[], None], destroy: Callable[[], None]) -> ctk.CTk:
    ...
    root.title(f'{roop.metadata.name} {roop.metadata.version} fixed by @vagmr')
    root.configure()
...
def select_output_path(start: Callable[[], None]) -> None:
    ...
    if is_image(roop.globals.target_path):
        output_path = ctk.filedialog.asksaveasfilename(title='save image output file', defaultextension='.png', initialfile=f'{out_png_name}-{out_time}.png', initialdir=RECENT_DIRECTORY_OUTPUT)
    elif is_video(roop.globals.target_path):
        output_path = ctk.filedialog.asksaveasfilename(title='save video output file', defaultextension='.mp4', initialfile=f'{out_video_name}-{out_time}.mp4', initialdir=RECENT_DIRECTORY_OUTPUT)
    else:
        output_path = None
    if output_path:
